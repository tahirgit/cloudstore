 	   ...
 	   if( isset($_POST['request_type']) )
	   {
		   # Setting remaining Important Fields
			...
			if($ticketModel->validate() && $ticketModel->save()) 
			{
				...
				# 2 To creator
				$message_creator = 'A new ticket to modify VDI user has been successfully created by '.Yii::app()->user->fullname.'
				 with Centrica IT. 
				...
				$message_support = 'A new ticket to modify VDI user has been successfully created by '.Yii::app()->user->fullname.'
				 with Centrica IT. 
 	   ...
 	   if( isset($_POST['request_type']) )
	   {
		   # Setting remaining Important Fields
			...
			if($ticketModel->validate() && $ticketModel->save()) 
			{
				...
				$ticketModel->updateMemId(); // Update Member ID in Tickets
				# 2 To creator
				$message_creator = 'A new ticket to modify Mailbox user has been successfully created by '.Yii::app()->user->fullname.' with Centrica IT. 
				Please login to <a href="https://mycloud.centrica-it.com/">customer panel</a> for 
									future correspondence. Ticket details are as follows:<br>
									<p> Ticket ID: '.$ticketModel->id.'<br>
										Priority: '.$this->possible_priority[$ticketModel->priority].'<br>
				...
				$message_support = 'A new ticket to modify Mailbox user has been successfully created by '.Yii::app()->user->fullname.' with Centrica IT. 
									Ticket details are as follows:<br>
									<p> Ticket ID: '.$ticketModel->id.'<br>
										Priority: '.$this->possible_priority[$ticketModel->priority].'<br>
										Created: '.date("M d, Y", $ticketModel->date).'<br>
   ...
   public function actionNewVdiUser()
   {
 	   ...
 	   if( isset($_POST['username']) )
	   {
		   # Setting remaining Important Fields
			...
			$ticketModel->subject			=  "Create New VDI User Request"; 
			$ticketModel->fk_customer_id 	=  Yii::app()->user->pk_customer_id;
			$ticketModel->priority 			=  4; // Urgent Ticket
			# validate user input and redirect to the previous page if valid
			...
			if($ticketModel->validate() && $ticketModel->save()) 
			{
				...
				# 2 To creator
				$message_creator = 'A new ticket to create VDI user has been successfully placed by '.Yii::app()->user->fullname.' 
				with Centrica IT. 
				Please login to <a href="https://mycloud.centrica-it.com/">customer panel</a> for 
									...
									<p> Ticket ID: '.$ticketModel->id.'<br>
										...
										Created: '.date("M d, Y", $ticketModel->date).'<br>
										Subject: '.$ticketModel->subject.'<br>
										'.$ticketModel->message.'<br>
 									</p>';
				$message_support = 'A new ticket to create VDI user has been successfully placed by '.Yii::app()->user->fullname.' with Centrica IT. 
									Ticket details are as follows:<br>
									<p> Ticket ID: '.$ticketModel->id.'<br>
										Priority: '.$this->possible_priority[$ticketModel->priority].'<br>
										Created: '.date("M d, Y", $ticketModel->date).'<br>
 	   ...
 	   if( isset($_POST['username']) )
	   {
		   # Setting remaining Important Fields
			...
			$ticketModel->message 			=  'Username: '.$request->getPost('username').'<br>
												Password: '.$request->getPost('password').'
												<br>Domain: '.$request->getPost("domain_name"); 
			if( isset($_POST['default_quota']))
			$ticketModel->message .= "<br> Default Quota: ".$_POST['default_quota'] > 0?$_POST['default_quota']:"N/A";											   
			if( isset($_POST['email_forward'])) 
			{ $ticketModel->message .= "<br> Email Forwarding To: ".strlen($_POST['email_forward']) > 3?$_POST['email_forward']:"N/A";}
			if( isset($_POST['free_backup_user'])) { $ticketModel->message .= "<br> Mailbox Free Backup Request: Enable";}
														
			$ticketModel->subject			=  "Create New Mailbox User Request"; 
			$ticketModel->fk_customer_id 	=  Yii::app()->user->pk_customer_id;
			$ticketModel->priority 			=  4; // Urgent Ticket
			...
			if($ticketModel->validate() && $ticketModel->save()) 
			{
				...
				# 2 To creator
				$message_creator = 'A new ticket to create Mailbox user has been successfully placed by '.Yii::app()->user->fullname.' 
				with Centrica IT. 
				Please login to <a href="https://mycloud.centrica-it.com/">customer panel</a> for 
									future correspondence. Ticket details are as follows:<br><br>
									<p> Ticket ID: '.$ticketModel->id.'<br>
				...
				$message_support = 'A new ticket to create Mailbox user has been successfully placed by '.Yii::app()->user->fullname.' 
				with Centrica IT. 
									Ticket details are as follows:<br>
									<p> Ticket ID: '.$ticketModel->id.'<br>
										Priority: '.$this->possible_priority[$ticketModel->priority].'<br>
